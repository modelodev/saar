////
//// Mission: translate between A2A wire payloads and SAD core types.
////
//// Responsibilities:
//// - Decode A2A message requests into `sad/types/input.InputPayload`.
...
////
//// Relationships:
//// - Used by `sad/gateway/a2a_api` (S16).
...
/// Converts a core SAD error to an A2A RFC7807 payload.
pub fn sad_error_to_a2a_error(err: types_output.SadError) -> #(Int, json.Json) {
...
/// Returns an SSE `data:` frame formatted by `sad/sse.line`.
...
pub fn agent_card_from_instance(
  ...
  json.object([
    ...
    #(
      ...
      json.array(
        [
          json.string("urn:sad:extensions:files-semantics:v1"),
        ],
        fn(item) { item },
      ),
    ),
