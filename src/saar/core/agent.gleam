...
////
//// Relationships:
//// - `saar/core/registry` stores and monitors `AgentRef` via `pid`.
//// - Internal event injection uses the `internal_*` functions below.
//// - Delegates pure lifecycle transitions to `saar/core/agent_lifecycle`.
...
import gleam/string
import saar/bridge/interaction
...
import saar/types/resolved_params.{type ResolvedParams}
...
///
/// This type is internal to SAAR and may contain sensitive data (resolved params).
...
fn log_buffer_limit_bytes(cfg: types_config.SaarConfig) -> Int {
  let types_config.SaarConfig(limits: limits, ..) = cfg
  let types_config.SaarLimits(log_buffer_bytes: log_buffer_bytes, ..) = limits
  log_buffer_bytes
}
...
pub fn resolve_call_timeout_for(
  ...
  let types_config.SaarTimeouts(call_timeout_ms: default_timeout_ms, ..) =
    timeouts
...
fn handle_worker_down(
  ...
  case state.mode {
    ...
    Busy(in_flight) -> {
      ...
      let err =
        types_output.saar_error(trace_id, types_enums.InfraError, "worker_down")
    ...
    Busy(in_flight) -> {
      ...
      case in_flight_trace == trace_id {
        ...
        True -> {
          ...
          let err =
            types_output.saar_error(trace_id, types_enums.InfraError, "timeout")
          send_in_flight_reply(in_flight, Error(err))
          actor.continue(clear_in_flight(state))
        }
      }
...
fn cancel_if_busy(state: AgentRuntimeState, code: String) -> AgentRuntimeState {
  case state.mode {
    ...
    Busy(in_flight) -> {
      ...
      let err = types_output.saar_error(trace_id, types_enums.AgentError, code)
      send_in_flight_reply(in_flight, Error(err))
...
pub fn status_timeout_ms(cfg: types_config.SaarConfig) -> Int {
  let types_config.SaarConfig(timeouts: timeouts, ..) = cfg
...
fn unwrap_or_disconnected(
  ...
  case out {
    ...
    Error(safe_call.TimedOut) ->
      Error(types_output.saar_error(trace_id, types_enums.InfraError, "timeout"))
  }
}
