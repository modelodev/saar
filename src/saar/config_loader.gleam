//// SAAR TOML configuration loader.
////
//// Mission: load `SaarConfig` from a TOML file with strict key validation and
...
//// - Apply defaults from `saar/types/config.default_saar_config`.
////
...
////
//// Relationships:
//// - Produces `saar/types/config.SaarConfig` consumed by bootstrap code.
...
/// `--config` > `SAAR_CONFIG_PATH` > `./config.toml`.
///
/// Example:
/// ```gleam
...
/// Loads and validates a `SaarConfig` from disk.
///
/// Defaults are taken from `types_config.default_saar_config()`.
///
...
pub fn load_from_path(
  ...
  let cfg = decode_with_defaults(parsed, types_config.default_saar_config())
...
fn is_api_key_present(cfg: types_config.SaarConfig) -> Bool {
...
fn decode_with_defaults(
  root: Dict(String, tom.Toml),
  defaults: types_config.SaarConfig,
) -> Result(types_config.SaarConfig, ConfigLoadError) {
  let cfg = defaults
...
fn apply_auth(
  ...
  case dict.get(root, "auth") {
    ...
    Ok(v) -> {
      ...
      let types_config.SaarConfig(api_key: old_key, ..) = cfg
      let next = case api_key {
        Some(value) -> types_core.secret_value(value)
        None -> old_key
      }

      Ok(types_config.SaarConfig(..cfg, api_key: next))
    }
  }
}
...
fn apply_profiles(
  ...
  case dict.get(root, "profiles") {
    ...
    Ok(v) -> {
      ...
      Ok(types_config.SaarConfig(..cfg, profiles: next_profiles))
    }
  }
}
...
fn apply_runners(
  ...
  case dict.get(root, "runners") {
    ...
    Ok(v) -> {
      ...
      Ok(types_config.SaarConfig(..cfg, runner: next_runner))
    }
  }
}
...
fn apply_workspaces(
  ...
  case dict.get(root, "workspaces") {
    ...
    Ok(v) -> {
      ...
      let types_config.SaarConfig(storage: storage_cfg, ..) = cfg
      ...
      Ok(types_config.SaarConfig(..cfg, storage: next_storage))
    }
  }
}
...
fn apply_limits(
  ...
  case dict.get(root, "limits") {
    ...
    Ok(v) -> {
      ...
      use saar_limits_overrides <- result.try(read_saar_limits_overrides(v))
      ...
      Ok(
        ...
        |> apply_saar_limits_overrides(saar_limits_overrides)
...
fn read_saar_limits_overrides(
  v: tom.Toml,
) -> Result(SaarLimitsOverrides, ConfigLoadError) {
...
fn apply_saar_limits_overrides(
  cfg: types_config.SaarConfig,
  overrides: SaarLimitsOverrides,
) -> types_config.SaarConfig {
  let types_config.SaarConfig(limits: limits_cfg, ..) = cfg
...
fn apply_stream_overrides(
  ...
  let types_config.SaarConfig(stream: stream_cfg, ..) = cfg
  ...
  types_config.SaarConfig(..cfg, stream: next_stream)
}
...
fn apply_log_stream(
  ...
  case dict.get(root, "log_stream") {
    ...
    Ok(v) -> {
      ...
      Ok(types_config.SaarConfig(..cfg, stream: next_stream))
    }
  }
}
...
fn is_absolute_workspaces_dir(cfg: types_config.SaarConfig) -> Bool {
