%% Custom SIGTERM handler for SAAR.
%%
%% This module is installed as a `gen_event` handler on `erl_signal_server`.
%% It forwards `sigterm` events to a target process as `{saar_sigterm}` and does
%% not call `init:stop/0` directly.

-module(saar_signal_handler).
