...
////
//// Mission: coordinate graceful shutdown for the SAAR HTTP gateway.
////
//// Responsibilities:
...
//// - Implementing HTTP routing (owned by `saar/gateway/http_server`).
//// - Persisting state across restarts.
////
//// Relationships:
//// - Started under `saar/core/root_supervisor`.
//// - Consulted by `saar/gateway/http_server` on each request.
//// - Uses `saar/core/shutdown_all` to terminate agents.
...
pub fn start(
  ...
  let init = fn(self) {
    ...
    let selector =
      ...
      |> process.select(self)
      |> process.select_record(atom.create("saar_sigterm"), 0, fn(_dyn) {
        BeginShutdown
      })
