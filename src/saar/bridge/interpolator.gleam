//// Template interpolation for runner command arguments and environment.
////
//// Mission: replace `{{param.name}}` placeholders in command args, env values,
//// and URLs with resolved parameter values.
////
//// Responsibilities:
//// - Parse placeholder syntax and resolve parameter references.
//// - Apply JSON pointer resolution for nested parameter access.
//// - Support input payload references (`{{input.*}}`).
////
//// Non-responsibilities:
//// - Parameter resolution from sources (handled by `saar/params`).
//// - Validating parameter schemas.
////
//// Relationships:
//// - Used by `saar/bridge/runner` to build final command invocations.
//// - Uses `saar/json_pointer` for nested access.
...
/// Address information for runners that need host/port binding.
...
fn to_saar_error(
  ...
  types_output.saar_error(trace_id, kind, interpolation_error_to_string(err))
}

pub fn interpolate_string(
  template: String,
  ctx: InterpContext,
) -> Result(String, types_output.SaarError) {
  interpolate_string_pure(template, ctx)
  |> result.map_error(fn(err) { to_saar_error(ctx.context.trace_id, err) })
}
...
pub fn interpolate_dict(
  ...
  ctx: InterpContext,
) -> Result(Dict(String, String), types_output.SaarError) {
...
pub fn interpolate_list(
  ...
  ctx: InterpContext,
) -> Result(List(String), types_output.SaarError) {
...
pub fn interpolate_json(
  template: json.Json,
  ctx: InterpContext,
) -> Result(json.Json, types_output.SaarError) {
...
pub fn resolve_json_pointer(
  ...
  |> result.map_error(fn(err) { to_saar_error(trace_id, err) })
}
...
fn helpers_to_value(payload: types_input.InputPayload) -> InterpValue {
  let types_input.SaarHelpers(last_user_content, last_user_files) =
