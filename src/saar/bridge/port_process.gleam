...
////
//// Responsibilities:
//// - Start the wrapper process via `saar/ffi` with the configured limits.
//// - Send control lines and read JSONL output with size checks.
//// - Delegate JSONL framing to `saar/bridge/jsonl_framer`.
////
...
////
//// Relationships:
//// - Uses `saar/ffi` for port IO and `gleam/erlang/application` for priv lookup.
//// - Uses `saar/bridge/jsonl_framer` for newline framing.
//// - Consumed by `saar/bridge/runner` as the port boundary.
...
fn wrapper_candidates() -> List(String) {
  let priv_candidate = case application.priv_directory("saar") {
    Ok(dir) -> [filepath.join(dir, "saar_wrapper")]
    Error(_) -> []
  }

  list.append(priv_candidate, ["./priv/saar_wrapper"])
}
