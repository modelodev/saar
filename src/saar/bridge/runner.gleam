...
////
//// Mission: start runners (transient or continuous), feed them input, and
//// collect their JSONL output into typed results.
////
//// Responsibilities:
//// - Start wrapper/runner processes with the correct env and args.
//// - Write input payloads to stdin and read JSONL from stdout.
//// - Enforce timeouts and collect artifacts.
//// - Handle both transient (one-shot) and continuous (server) lifecycles.
////
//// Non-responsibilities:
//// - HTTP routing or agent lifecycle (handled by `saar/core/agent`).
//// - Profile resolution or parameter sources.
////
//// Relationships:
//// - Used by `saar/bridge/interaction` and `saar/bridge/port_owner`.
//// - Uses `saar/bridge/port_process` for port I/O.
//// - Uses `saar/bridge/runner_contract` for event decoding.
...
fn instance_id_from_input(
  ...
  let types_input.SaarInput(meta: meta, ..) = input
...
fn interaction_error(
  ...
  types_output.saar_error(trace_id, types_enums.InfraError, message)
}
