...
def collect_artifacts(workspace: Path, config: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    for path in workspace.rglob("*"):
        ...
        if not any(glob_match(pat, rel) for pat in include_patterns):
            continue
        if any(glob_match(pat, rel) for pat in exclude_patterns):
            continue
        artifacts.append({
            "name": path.name,
            "path": rel,
...
def match_segments(patterns: List[str], paths: List[str]) -> bool:
    if not patterns:
        return paths == []
    head, *rest = patterns
    if head == "**":
        return match_double_star(rest, paths)
    if not paths:
        return False
    return match_segment(head, paths[0]) and match_segments(rest, paths[1:])


def match_double_star(rest: List[str], paths: List[str]) -> bool:
    if not rest:
        return True
    return match_double_star_step(rest, paths)


def match_double_star_step(rest: List[str], paths: List[str]) -> bool:
    if match_segments(rest, paths):
        return True
    if not paths:
        return False
    return match_double_star_step(rest, paths[1:])


def match_segment(pattern: str, segment: str) -> bool:
    return match_chars(list(pattern), list(segment))


def match_chars(pattern: List[str], text: List[str]) -> bool:
    if not pattern:
        return text == []
    head, *rest = pattern
    if head == "*":
        if not text:
            return match_chars(rest, [])
        return match_chars(rest, text) or match_chars(pattern, text[1:])
    if head == "?":
        if not text:
            return False
        return match_chars(rest, text[1:])
    if not text:
        return False
    return head == text[0] and match_chars(rest, text[1:])
