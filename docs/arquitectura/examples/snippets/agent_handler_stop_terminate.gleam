fn handle_stop_instance(
  ...
  let selector = case state.mode {
    ...
    Busy(in_flight) -> {
      ...
      // Cancelar el worker
      let AgentDeps(_artifact_registry, _port_pool, _registry, bridge) =
        state.deps
      bridge.cancel_interaction(handle)

      // Responder error al cliente (ReplyChannel es alias de Subject)
      let err =
        InteractionError(InfraError, "Agent shutting down", req.trace_id)
  ...
  case state.assigned_port {
    Some(_) -> {
      let AgentDeps(_artifact_registry, port_pool, _registry, _bridge) =
        state.deps
