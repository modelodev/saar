    ...
    case streaming {
      ...
      False ->
        agent.internal_interaction_done(
          agent,
          execute_http_capability(ctx, req, timeout_ms),
        )
    }
  })
}
    ...
    Some(sink) -> {
      case
        stream_sink.push_batch(
          sink,
          [stream_started(trace_id)],
          stream_cfg.push_timeout_ms,
        )
      {
        Ok(_) -> Some(sink)
        Error(_) -> None
...
fn finish_streaming_without_connection(
  ...
  case stream_sink_option {
    Some(sink) ->
      stream_sink.finish(sink, Error(err), stream_cfg.push_timeout_ms) |> ignore
    None -> Nil
  }
...
fn execute_http_streaming(
  ...
  let stream_sink_option =
    stream_sink_after_started_event(
      stream_sink_option,
      req.trace_id,
      stream_cfg,
    )
  ...
  case setup_sse_connection(ctx, req) {
    Error(err) ->
      finish_streaming_without_connection(
        agent,
        stream_sink_option,
        stream_cfg,
        err,
...
fn setup_sse_connection(
  ctx: BridgeCtx,
  req: AgentRequest,
) -> Result(SseConnection, InteractionError) {
  case ctx.profile.interface {
    RunnerInterface(_) ->
      Error(InteractionError(
        InfraError,
        "HTTP streaming on runner interface",
        req.trace_id,
      ))
        ...
        _ -> {
          ...
          use url <- result.try(
            ...
            |> result.map_error(fn(e) {
              InteractionError(InfraError, e, req.trace_id)
            }),
          )
            ...
            Ok(RunnerEventChunk(delta)) -> {
              let stream_sink_option =
                push_stream_event(
                  stream_sink_option,
                  stream_cfg,
                  content_chunk(ctx.trace_id, delta),
                )
              sse_read_loop(
                conn,
                agent,
  ...
  case ctx.profile.interface {
    ...
    HttpInterface(base_url, headers, _, capabilities) -> {
      ...
      case dict.get(capabilities, req.capability) {
        ...
        Ok(cap) ->
          execute_capability(base_url, headers, cap, req, ctx, timeout_ms)
      }
    }
  }
}
...
fn parse_http_response(
  ...
  case response.status >= 200 && response.status < 300 {
    ...
    True -> {
      ...
      Ok(InteractionResult(data: data, artifacts: [], trace_id: trace_id))
    }
  }
}
