...
pub fn request(
  ...
  let req = request.new()
    |> request.set_method(method)  // Ya es Method, no parse
  ...
  case send.send(req) {
    ...
    Error(hackney.ConnectionClosed(_)) -> Error(ConnectionError("Connection closed"))
    Error(hackney.InvalidUtf8Response) -> Error(Unexpected("Invalid UTF-8 in response"))
