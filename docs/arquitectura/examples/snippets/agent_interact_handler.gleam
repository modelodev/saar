...
fn handle_interact(
  ...
  case is_failed(state.state) {
    True -> {
      let reason = get_failure_reason(state.state)
        |> option.unwrap("unknown")
      let err = InteractionError(InfraError, "Agent failed: " <> reason, req.trace_id)
  ...
  case state.mode {
    ...
    Idle -> {
      case is_ready(state.state) {
        False -> {
          let err = InteractionError(InfraError, "Agent not ready", req.trace_id)
          process.send(reply_to, Error(err))
          actor.continue(state)
        }
...
fn start_interaction(
  ...
  req: AgentRequest,
  stream_sink: Option(StreamSink),
  reply_to: ReplyChannel,  // ReplyChannel es alias de Subject(Result(...))
) -> actor.Next(AgentRuntimeState, AgentMsg) {
  let self = process.self()
  
  // Determinar si el caller pidió streaming (tiene sink) y si la capability lo soporta.
  let capability_streaming = is_streaming_capability(state.profile.interface, req.capability)
  ...
  let timeout_ms = resolve_call_timeout_for(state.config, state.profile.interface, req.capability)
  ...
  let in_flight = InFlight(req, reply_to, handle)  // reply_to es directamente el Subject
...
fn is_streaming_capability(interface: Interface, capability_name: String) -> Bool {
  case interface {
