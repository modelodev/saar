...
Un ejemplo típico de fallo asíncrono es `PORT_POOL_EXHAUSTED` (continuous con `managed_port` sin puertos libres), que se observa como `phase=failed` + `failure_reason` en status.
