...
SAD es un **adaptador universal de agentes**. Resuelve un problema fundamental: cada agente de IA (Aider, Vanna, GPT-Researcher, LightRAG, etc.) tiene su propia forma de ejecutarse y comunicarse. SAD abstrae esa heterogeneidad y expone una interfaz homogénea.
...
El cliente habla un único protocolo. SAD traduce.
...
SAD es **infraestructura**, no un producto para usuario final. Sus usuarios son:

| Usuario | Qué hace con SAD |
|---------|------------------|
...
- `bridge.md`: streaming data-plane **solo** worker → `sad/streams/sink.StreamSink` con batching (`InteractionStreamConfig`) y degradación a discard (sin buffers infinitos).
- `gateway.md`: `sad/streams/sink.StreamSink` por request streaming (SSE writer) + ack/timeout; disconnect no cancela ejecución.
...
**Nota:** `AgentRef` es un handle **en memoria** (válido solo dentro del nodo SAD). No cruza HTTP ni puede ser conocido por SAM.
SAM identifica instancias por `instance_id`; el gateway resuelve `instance_id → AgentRef` vía registry.

## 6. Estados del agente

```gleam
// Estado observable (wire/diagnóstico). Ubicación: `sad/types.gleam`
pub type AgentPhase {
  Created
  Provisioning
  ReadyTransient
...
- `sad/types.gleam` es **dominio + wire** y no importa `gleam/erlang/*` ni `gleam/otp/*` (sin `Subject`, `Pid`, `Monitor`, `Selector`, `Port`).
- `sad/core/messages.gleam` concentra **mensajería OTP** (Subjects, ReplyChannels, handles, monitors).
- `sad/core/*` contiene estado y actores/supervisión (control-plane).
- `sad/bridge/*` contiene IO pesado (ports, HTTP).
- `sad/gateway/*` contiene router HTTP y handlers (incluye `ui_proxy.gleam` separado).
- `sad/adapters/*` contiene mapeos wire (A2A, AG-UI, A2UI).
- `sad/streams/*` contiene sink/batching/backpressure.
...
|-----------|-------------|--------------|
| Interacción (transient + continuous) | `default_sad_config().call_timeout_ms` | Sí (y se puede sobrescribir por capability limits) |
| Health check (continuous) | `default_sad_config().health_check_timeout_ms` | Sí |
| Shutdown (stop/delete) | `default_sad_config().shutdown_timeout_ms` | Sí |
