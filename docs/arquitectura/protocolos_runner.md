...
- SAAR siempre invoca `wrapper -- <runner> ...` (ver `arquitectura/examples/wrapper_pid_namespace.md`).
- Wrapper crea PID+user namespace, lanza el runner y gestiona control por stdin (JSONL):
  - `{"t":"input","payload":<SAAR_INPUT_JSON>}` (línea única): reenvía el `payload` al runner por stdin y luego cierra su stdin.
  - `{"t":"stop"}` (línea única) o EOF → SIGTERM → timeout → SIGKILL a la subtree.
- SAAR no necesita FFI de señales ni conocer `os_pid`; la FFI solo cubre open/send/close/receive del port (idealmente vía `saar/bridge/port_process.gleam`).
...
### Entradas y salidas
- STDIN del wrapper: JSONL de control. Primera línea: `{"t":"input","payload":<SAAR_INPUT_JSON>}`. Líneas posteriores: `{"t":"stop"}` (o futuros comandos). EOF equivale a stop.
- STDIN del runner: `SAAR_INPUT_JSON` (validado) para start/exec; el wrapper lo reenvía y luego cierra stdin.
- STDOUT: **stream de eventos** (una línea por evento, JSONL/NDJSON). Incluye logs, streaming y el resultado final.
- STDERR: fuera de contrato (diagnóstico local). SAAR no debe depender de capturarlo ni de separarlo de STDOUT.

**Racional (OTP/BEAM):** `open_port` no ofrece separación real de stdout/stderr cuando se usa `use_stdio`. Para evitar complejidad (wrappers multiplexores, `erlexec`, etc.), SAAR trata la salida capturada como **un único canal** y la separación se hace **a nivel de protocolo** (eventos tipados).
...
- **`saar runner-test` como gate:** cualquier runner/wrapper debe pasar `saar runner-test` (ideal: CI obligatorio) para evitar “basura” accidental en STDOUT.
- **Adaptadores fuera del core:** si queréis ejecutar un binario que escribe texto libre, haced un “shim runner” que capture su salida y emita eventos JSONL; ese shim vive fuera del core SAAR.
- **Límite de control:** el wrapper corta si una línea de control excede `SAAR_WRAPPER_CONTROL_LINE_BYTES` (default 262_144).
...
- Streaming (solo si la capability tiene `streaming: true`):
  ...
  - El bridge traduce `chunk.delta` a `ContentChunk` y lo entrega por SSE por batches vía `saar/streams/sink.StreamSink` (sin buffers infinitos).

### Protocolo de eventos en STDOUT (JSONL)

Todos los eventos se emiten como **una línea JSON por evento**. SAAR parsea línea a línea (fail-fast si el runner emite bytes que no forman líneas válidas bajo el límite configurado).
...
Si se excede, SAAR aborta la interacción por violación del contrato (protege de fragmentación/overflow en ports).
...
### Stop
- SAAR envía la línea `{"t":"stop"}` (terminada en `\n`) y/o cierra stdin.
...
- Valores configurables: `shutdown_timeout_ms` (SAAR) y `SAAR_WRAPPER_POST_KILL_WAIT_MS`/`SAAR_WRAPPER_POLL_MS` (wrapper).
