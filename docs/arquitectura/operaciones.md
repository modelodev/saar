...
**Decisión:** Toda FFI en `saar/ffi.gleam`.

**Qué requiere FFI:**
- `now_ms()` — timestamps
 - **Ports básicos (open, send, close, receive)** mediante un shim Erlang mínimo (`saar_ffi.erl`) invocado vía `@external` en `saar/ffi.gleam`.
...
**Decisión para ports:**
- FFI mínima para `open_port`/send/close/receive, encapsulada detrás de un módulo frontera (`saar/bridge/port_process.gleam`) para que el resto del sistema no dependa de Erlang.
...
**Decisión:** SAAR no maneja S3/GCS. Artefactos en volumen montado.
...
- Tipos **dominio/wire** (serializables, sin OTP) viven en `saar/types.gleam`.
- Tipos **runtime/OTP** (p.ej. `AgentState`, `AgentRuntimeState`, `ActorMode`, `AgentResource`) viven en sus módulos (`saar/core/*`) y **no** se exponen por HTTP.
...
**Nota operativa (seguridad):** SAAR no se considera una API pública. Aun así, se asume que puede quedar expuesto accidentalmente; por eso v0 aplica auth por API key y endurece `/artifacts` y `/ui`. Recomendación: exponer SAAR solo detrás de una capa superior (SAM/reverse proxy) y limitar alcance de red.

### ADR-007: Cancelación (fuera de v0)

**Decisión v0:** No hay cancelación explícita ni implícita por desconexión de clientes. Si el cliente SSE se desconecta, SAAR deja de emitir eventos pero la interacción sigue y el runner no se mata salvo timeout/errores. Principio: SAAR no toma decisiones sobre el agente sin orden superior.

### ADR-008: Puertos dinámicos y wrapper controlador

**Decisión:** Solo `managed_port` + `no_network`. SAAR usa un port pool (rango explícito, `min>0` y `max>=min`) e inyecta host/port via env vars. Se elimina `static_port`.

**Kill de runners:** Los runners se lanzan siempre a través de un wrapper que crea PID namespace (rootless vía user namespace), lanza el proceso real y, al recibir `stop` (mensaje o EOF), envía SIGTERM→timeout→SIGKILL a toda su subtree. SAAR cierra stdin/envía comando; el wrapper garantiza limpieza sin que SAAR tenga que gestionar `os_pid`. Si el wrapper detecta muerte del parent (port owner), debe auto-terminar su subtree.
...
- SAAR solo cierra stdin del port o envía un comando `stop` al wrapper y espera `shutdown_timeout_ms`.
...
**Decisión:** SAAR v0 evita “background jobs” internos. Un endpoint es:
...
**Razón:** reduce código propio de framing/keep-alive/cierre, mantiene BEAM-idiomatic (un proceso dueño del socket) y deja el core agnóstico (el bridge solo habla con `saar/streams/sink.StreamSink`).

---

## 3. CLI

SAAR es un **binario único** con subcomandos. No hay `saar-cli` separado.
...
saar serve
saar serve --port 9000
saar serve --config /etc/saar/config.toml

# Arrancar en background
saar serve -b
saar serve --background
saar serve -b --port 9000
...
saar serve -k && saar serve -b

# Ver estado
saar serve --status
# Output: SAAR running on port 8080 (PID 12345)
# Output: SAAR not running
```

**Comportamiento de `-b` (background):**
1. Fork del proceso
2. Escribe PID a `~/.saar/saar.pid` (o `$SAAR_PID_FILE`)
3. Redirige logs a `~/.saar/saar.log` (o `$SAAR_LOG_FILE`)
4. Proceso padre termina inmediatamente

**Comportamiento de `-k` (kill):**
1. Lee PID de `~/.saar/saar.pid` (o `$SAAR_PID_FILE`)
2. Envía `SIGTERM` al proceso **SAAR** (no a runners)
...
- SAAR reemplaza el handler por defecto de Erlang para SIGTERM (`saar/ffi/signals.gleam` + `saar/ffi/saar_signal_handler.erl`) y reenvía el evento al proceso `GatewayShutdown`.
...
```bash
# Validar perfiles
saar validate ./profiles/aider.json
saar validate ./profiles/            # Valida todos en directorio

# Dry-run (simula sin ejecutar)
saar dry-run ./profiles/aider.json --input payload.json --capability chat
...
```bash
# Requeridas
export SAAR_API_KEY="your-secret-key"

# Opcionales - config
export SAAR_CONFIG_PATH="/etc/saar/config.toml"  # Default: ./config.toml
export SAAR_LOG_LEVEL="info"                     # debug|info|warn|error
export SAAR_PORT="8080"                          # Override de config

# Opcionales - daemon
export SAAR_PID_FILE="~/.saar/saar.pid"           # PID file para -b/-k
export SAAR_LOG_FILE="~/.saar/saar.log"           # Log file para -b
```
...
saar serve 2>&1 | tee /var/log/saar/saar.log

# Background con redirección
saar serve >> /var/log/saar/saar.log 2>&1 &

# Formato de logs
# [INFO] SAAR v3.0.0 starting...
...
```ini
# /etc/systemd/system/saar.service
[Unit]
Description=SAAR - Sofias Agent Driver
...
User=saar
Group=saar
Environment="SAAR_API_KEY=your-secret-key"
Environment="SAAR_CONFIG_PATH=/etc/saar/config.toml"
ExecStart=/usr/local/bin/saar serve
Restart=on-failure
RestartSec=5
...
```bash
# Gestión con systemd
sudo systemctl enable saar
sudo systemctl start saar
sudo systemctl status saar
sudo systemctl stop saar
journalctl -u saar -f  # Ver logs
```
...
ENV SAAR_API_KEY=""
ENV SAAR_CONFIG_PATH="/app/config.toml"
EXPOSE 8080

CMD ["gleam", "run", "-m", "saar", "--", "serve"]
```

```bash
docker run -d \
  -p 8080:8080 \
  -e SAAR_API_KEY="secret" \
  -v ./profiles:/app/profiles \
  -v ./config.toml:/app/config.toml \
  saar:latest
```

### 3.7 Validaciones de `saar validate`
...
`saar runner-test --contract` verifica:
...
| Timeouts | ✅ v0 | Defaults en `default_saar_config()`; en producción se ajustan vía `config.toml` |
